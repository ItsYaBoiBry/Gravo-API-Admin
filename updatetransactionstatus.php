    ...
    $query_changepassword = "UPDATE transaction SET status_id = $status, driver_arrival_time = '$arrivalTime' WHERE id = $transactionid;";
    ...
    if ($conn) {
        if ($changepassword) {
            ...
            if ($gettransactions != []) {
                if($status == "2"){
                    JSONResponse(201, "Success, Driver on the way", $gettransactions);
                }else if($status == 3){
                    JSONResponse(200, "Success, Driver has arrived", $gettransactions);
                }
            } else {
                JSONResponse(404, "No transactions", $gettransactions);
            }
        } else {
            JSONResponse(400, "status not updated", $changepassword);
        }
    } else {
