...
if ($conn) {
    ...
    if ($checkitemexist != []) {
        $queryupdateitem = "UPDATE bulk_transaction_item SET bulk_transaction_status_id = 7 WHERE id = $getid;";
        if (changebulkstatus($conn, $queryupdateitem)) {
            $queryaddhistory = "INSERT INTO bulk_transaction_history (bulk_transaction_item_id, history) VALUES ($getid,'$getname has rejected the quoted price.')";
