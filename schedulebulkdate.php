...
if ($conn) {
    ...
    if ($checkitemexist != []) {
        $queryupdateitem = "UPDATE bulk_transaction_item SET bulk_transaction_status_id = 3, collection_date='$collectiondate', collection_date_timing = '$collectiondatetiming' WHERE id = $getid;";
        if (changebulkstatus($conn, $queryupdateitem)) {
            $queryaddhistory = "INSERT INTO bulk_transaction_history (bulk_transaction_item_id, history) VALUES ($getid,'$getname scheduled a collection date on $collectiondate at $collectiondatetiming.')";
            ...
            JSONResponse(400, "unable to update the transaction!", null);
        }
    } else {
